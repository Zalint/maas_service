...
(function (racine, fabrique) {
    ...
    /** Le nombre de jours du mois, annees bissextiles comprises. */
    function joursDuMois(annee, mois) {
        if (mois === 2) {
            var bis = (annee % 4 === 0 && annee % 100 !== 0) || annee % 400 === 0;
            return bis ? 29 : 28;
        }
        return [4, 6, 9, 11].indexOf(mois) >= 0 ? 30 : 31;
    }
     ...
     *
     * Le motif ne suffit pas: '2026-02-30' et '2027-02-29' ont la bonne FORME
     * sans exister. Les laisser passer contredirait le contrat de cette
     * fonction - rendre '' des qu'on ne sait pas - et ferait circuler une date
     * impossible jusque dans une soustraction de jours, ou elle donnerait un
     * ecart faux plutot qu'un null.
     */
    function jourISO(v) {
        if (typeof v !== 'string') return '';
        var m = v.match(ISO);
        if (!m) return '';
        var mois = parseInt(m[2], 10), jour = parseInt(m[3], 10);
        if (mois < 1 || mois > 12) return '';
        if (jour < 1 || jour > joursDuMois(parseInt(m[1], 10), mois)) return '';
        return m[0];
    }
