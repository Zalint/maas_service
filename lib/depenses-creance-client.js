...
/**
 * L'integration MataBanq est-elle CONFIGUREE sur ce deploiement ?
 *
 * A ne pas confondre avec "elle repond". fetchCreanceCdb rend null dans les
 * deux cas, et cette confusion coutait cher: le PL refusait de se figer sur
 * tout tenant qui n'utilise pas MataBanq, alors que ce mode est explicitement
 * supporte (.env.example: "Si DEPENSES_API_BASE_URL ou DEPENSES_API_KEY est
 * vide, le bloc Creance CDB reste masque cote UI").
 *
 * Non configuree, les avances valent legitimement zero et le PL est complet.
 * Configuree mais muette, il manque un poste et le PL est faux: c'est le seul
 * cas qui justifie de refuser un figeage.
 *
 * @returns {boolean}
 */
function estConfigure() {
    return aDesIdentifiants() && !!readFinanceLabel();
}

/**
 * Les IDENTIFIANTS sont-ils presents ? Distinct de estConfigure(), qui exige
 * en plus un libelle lisible.
 *
 * La distinction compte pour le figeage du PL. Confondre les deux laissait
 * passer un cas faux: identifiants presents mais brand-config.json sans
 * libelle, fetchCreanceCdb rend null, les avances valent 0 - et estConfigure()
 * rendant false, le PL se declarait 'non_configure' donc FIABLE, et se figeait
 * ampute. Or ce deploiement utilise bien MataBanq: son silence est une panne,
 * pas un choix.
 *
 * @returns {boolean}
 */
function aDesIdentifiants() {
    const baseUrl = (process.env.DEPENSES_API_BASE_URL || '').trim();
    const apiKey = (process.env.DEPENSES_API_KEY || '').trim();
