...
async function lireReglages() {
    ...
    try {
        ...
        return {
            ...
            familleBoeuf: DEFAUTS.familleBoeuf.slice(),
    ...
    let familleBoeuf = DEFAUTS.familleBoeuf.slice();
    if (brut[CLES.familleBoeuf] !== undefined) {
        const liste = parseListe(brut[CLES.familleBoeuf]);
...
function valider(corps) {
    ...
    if (corps.familleBoeuf !== undefined) {
        ...
        // seul cote.
        const r = validerListeDeNoms(corps.familleBoeuf, 'familleBoeuf');
        if (r.erreur) erreurs.push(r.erreur);
        else aEcrire.push({ key: CLES.familleBoeuf, value: r.liste.join(',') });
    }
