...
function valider(corps) {
    ...
    if (corps.famillePoulet !== undefined) {
        const liste = Array.isArray(corps.famillePoulet)
            ? corps.famillePoulet
            : parseListe(corps.famillePoulet);
        if (liste === null) {
            erreurs.push('famillePoulet doit être une liste de noms ou une chaîne séparée par des virgules');
        } else {
            const propres = liste.map((n) => String(n).trim()).filter(Boolean);
            // Borne defensive: la valeur est stockee en TEXT, mais une liste
            // de mille noms saisie par erreur rendrait l'ecran illisible.
            if (propres.length > 50) erreurs.push('famillePoulet : 50 produits au maximum');
            else aEcrire.push({ key: CLES.famillePoulet, value: propres.join(',') });
        }
    }
