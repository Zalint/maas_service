...
function construireContexte(sim, pl, commissionPct) {
    ...
    return {
        ...
        // Les produits dont le prix porte DEJA la commission MaaS: le serveur
        // ne la facture plus sur leurs livraisons, et la marge nette calculee
        // ici ne doit donc plus la deduire. Pris sur `pl`, la meme reponse que
        // le montant de commission ci-dessus - c'est elle qui l'explique - et
        // parce que le cas du bœuf depend de ce que DATA a publie a cette date
...
function calculerProjection({ sim, pl, commissionPct }) {
    ...
    // La marge nette de commission etait RECOPIEE ici, formule comprise, alors
    // que l'en-tete de ce fichier promet de n'en ecrire aucune. La promesse
    // est tenue depuis que le moteur la porte: une correction de l'assiette ne
    // peut plus s'appliquer a l'ecran sans s'appliquer a l'API.
    const margeApresCommission = (p) => M.margeApresCommission(p, contexte);
    ...
    return {
        hypotheses: {
            ...
            // Le taux seul ne suffit pas a refaire le calcul: il ne s'applique
            // pas aux produits dont le prix d'achat porte deja la commission.
            commission_integree: contexte.commissionIntegree,
