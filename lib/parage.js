...
function calculerParage(args) {
    ...
    for (const s of stocksMatin) {
        ...
        pour(s.pointVente)[cat].matin += q;
        pour(s.pointVente)[cat].theorique += q;
    }
    ...
    for (const t of transferts) {
        ...
        pour(t.pointVente)[cat].transferts += q;
        pour(t.pointVente)[cat].theorique += q;
    }
    ...
    for (const acc of Object.values(parPv)) {
        for (const cat of CATEGORIES) {
            const d = acc[cat];
            // Seul un denominateur nul ou negatif rend le parage incalculable:
            // il n'y avait rien a mesurer, et on affichera un tiret.
            //
            // Du stock theorique SANS vente, en revanche, donne bien 100% - et
            ...
            d.ratio = d.theorique > 0 ? d.vendu / d.theorique : null;
