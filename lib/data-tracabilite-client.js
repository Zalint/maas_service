...
async function getLotActuel(opts) {
    ...
    _inflight = (async () => {
        ...
        try {
            // Node 18+ a fetch global. Timeout via AbortController, cleanup
            // dans finally pour ne jamais laisser le setTimeout traîner si
            // fetch throw avant clearTimeout.
            ...
            // fetchedAt au WRITE time, pas au temps du debut de fetch:
            // sinon la TTL effective est rognee par la duree du round-trip
            // (sur un appel de 3s, on perdrait 3s de cache).
