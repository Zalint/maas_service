...
function drapeauxDe(veille, jour, stock, recalc, margeJour) {
    const sV = veille.stock || {}, sJ = jour.stock || {};
    const out = [];

    // 00. LA MARGE DE LA JOURNEE EST NEGATIVE.
    //
    // MATA facture au COUT d'achat. Une journee de commerce normale verifie
    // donc ventes + variation de stock > avances + paiements: la marchandise
    // vendue rapporte plus qu'elle n'a coute. Quand l'inegalite s'inverse,
    // c'est qu'il est sorti du frigo plus de valeur que la caisse n'en a
    // encaisse, et il n'y a que trois causes possibles.
    //
    // Le drapeau passe en tete: c'est le plus grave, et il explique a lui seul
    // la plupart des journees negatives. Constate le 24/08/2026 a Keur Massar,
    // ou 40 kg de poisson livres et factures 96 000 F manquaient au stock du
    // soir; une fois saisis, la marge du jour est repassee de -52 369 a
    // +43 631.
    const mj = margeJour || {};

    // 0. UNE AVANCE N'A PAS ENCORE ETE SAISIE.
    //
    // Le symetrique du drapeau ci-dessous. Quand le stock MONTE alors
    // qu'AUCUNE avance n'est enregistree, de la marchandise est entree sans
    // etre facturee: MATA facture au prix d'achat, une livraison laisse donc
    // toujours une avance derriere elle.
    //
    // Sans ce drapeau, la journee affiche une marge flatteuse et un taux
    // absurde - constate le 25/08/2026 a Mbao: ventes 125 000 + stock 60 847
    // - avances 0 = +185 847, soit 148,68 % des ventes. Un taux de marge
    // superieur a 100 % sur la marchandise n'existe pas; il ne dit rien
    // d'autre que la facture manquante. « Detail par date » le montrait deja
    // de son cote, en marquant la journee « aucune avance ».
    if (nb(mj.avances) === 0 && nb(mj.stock) > 0 && nb(mj.ventes) > 0) {
        out.push({ cle: 'avance_absente', niveau: 'fort',
            texte: 'AUCUNE AVANCE sur la journée alors que le stock a AUGMENTÉ de '
                + fmtFr(nb(mj.stock)) + '. MATA facturant au prix d’ACHAT, une '
                + 'livraison laisse toujours une avance derrière elle : celle-ci n’a '
                + 'probablement pas encore été saisie côté partenaire. Tant qu’elle '
                + 'manque, la marge du jour ('
                + (nb(mj.marge) >= 0 ? '+' : '') + fmtFr(nb(mj.marge))
                + (mj.taux_pct !== null && mj.taux_pct !== undefined
                    // Deux decimales, comme le bloc de marge juste au-dessus:
                    // arrondir 148,68 en 149 ferait douter du chiffre affiche.
                    ? ', soit ' + nb(mj.taux_pct).toFixed(2) + ' % des ventes'
                    : '')
                + ') est surévaluée d’autant, et le PL du jour avec elle.' });
    }

    if (mj.marge !== null && mj.marge !== undefined && nb(mj.marge) < 0) {
        const manque = Math.abs(nb(mj.marge));
        out.push({ cle: 'marge_negative', niveau: 'fort',
            texte: 'MARGE NÉGATIVE de ' + fmtFr(manque) + ' sur la journée : les avances '
                + '(' + fmtFr(Math.abs(nb(mj.avances))) + ')'
                + (nb(mj.paiements) !== 0
                    ? ' et les paiements fournisseur (' + fmtFr(Math.abs(nb(mj.paiements))) + ')'
                    : '')
                + ' dépassent les ventes (' + fmtFr(nb(mj.ventes)) + ') augmentées de la '
                + 'variation de stock (' + fmtFr(nb(mj.stock)) + '). MATA facturant au prix '
                + 'd\u2019ACHAT, cela ne devrait pas arriver sur une journée normale. '
                + 'Trois causes possibles : de la marchandise livrée et facturée n\u2019a pas '
                + 'été saisie au stock du soir, une vente n\u2019a pas été enregistrée, ou de '
                + 'la marchandise est réellement partie sans passer en caisse.' });
    }
...
function ecartJour(args) {
    ...
    // LA MARGE DE LA JOURNEE, isolee des charges.
    //
    // ventes + variation de stock - avances - paiements. C'est ce que la
    // journee a gagne sur la marchandise, avant commission, charges et
    // depenses. Le PL du jour, lui, melange les deux et on ne sait plus si
    // une journee negative vient du commerce ou de la structure.
    //
    // Les contributions portent DEJA leur signe (avances et paiements sont
    // en -1): la somme des quatre EST la marge, sans avoir a le refaire.
    const contribDe = (cle) => {
        const t = postes.find((x) => x.cle === cle);
        return t ? nb(t.contribution) : 0;
    };
    const margeJour = {
        ventes: contribDe('ventes'),
        stock: contribDe('stock'),
        avances: contribDe('avances'),
        paiements: contribDe('paiements'),
        // Positive = la marchandise sortie a rapporte plus qu'elle n'a coute.
        marge: round2(contribDe('ventes') + contribDe('stock')
            + contribDe('avances') + contribDe('paiements'))
    };
    // Le taux n'a de sens que s'il y a eu des ventes: sans elles, diviser par
    // zero rendrait Infinity, et l'afficher comme un taux serait pire que de
    // ne rien dire.
    margeJour.taux_pct = margeJour.ventes > 0
        ? round2((margeJour.marge / margeJour.ventes) * 100)
        : null;
    ...
    return {
        ...
        // La marge de la JOURNEE, marchandise seule. Rendue a part du PL du
        // jour, qui lui inclut commission, charges et depenses: une journee
        // peut tres bien gagner sur la marchandise et finir negative une fois
        // la structure payee, et l'inverse est le signe d'une anomalie.
        marge_jour: margeJour,
