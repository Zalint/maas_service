...
function ecartJour(args) {
    ...
    //
    //   variation nette = (stock de fin - stock de depart) x coefficient
    //
    // Identite verifiee sur les snapshots reels. Le stock de DEPART est celui
    // du 1er du mois: les deux cumuls le partagent, et tout l'ecart vient donc
    // du stock de FIN. L'afficher permet de refaire le calcul a la main au
    // lieu de croire deux nombres nets tombes du ciel.
    stock.bornes = {
        ...
        // Ce que la ligne du tableau doit valoir, refait depuis les bornes.
        variation_veille: round2((nb(sV.soir_fin) - nb(sV.matin_debut)) * nb(sV.coeff)),
        variation_jour: round2((nb(sJ.soir_fin) - nb(sJ.matin_debut)) * nb(sJ.coeff))
    };
