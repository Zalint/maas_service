...
function bloc() {
    return {
        ...
        // Bilan dechet cumule, meme structure qu'au jour (cf lib/parage.js):
        // la decomposition du mois doit se verifier comme celle du jour.
        dechet: { matin: 0, transferts: 0, soir: 0, vendu: 0, jete: 0, produit: 0 },
        taux_dechet: null, taux_deperdition: null
    };
}

// Champs du bilan dechet, additionnes terme a terme lors des cumuls.
const CHAMPS_DECHET = ['matin', 'transferts', 'soir', 'vendu', 'jete', 'produit'];
function cumulerDechet(cible, source) {
    if (!source) return;
    for (const k of CHAMPS_DECHET) cible.dechet[k] += source[k] || 0;
}
...
function cumulerMois(parageParJour) {
    ...
    for (const cat of CATEGORIES) {
        ...
        t.ratio = taux.ratio;
        t.perte = taux.perte;
        // Meme decomposition qu'au jour: global = dechet + deperdition, au
        // meme denominateur (cf lib/parage.js). Le produit est recalcule des
        // termes cumules - la formule est lineaire, la somme des bilans
        // journaliers donne le meme resultat.
        t.dechet.produit = t.dechet.soir + t.dechet.vendu + t.dechet.jete
            - t.dechet.matin - t.dechet.transferts;
        t.taux_dechet = t.theorique > SEUIL_KG ? t.dechet.produit / t.theorique : null;
        t.taux_deperdition = (t.perte === null || t.taux_dechet === null)
            ? null : t.perte - t.taux_dechet;
    }
    return total;
}
