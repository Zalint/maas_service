...
async function creerResolveurPrixAchat(dateMaxIso) {
    ...
    const pourDate = (dateIso) => {
        ...
        const resoudre = (produit) => {
            // 1. Le prix PROPRE du produit gagne toujours. Foie et Yell sont
            //    achetes separement de la carcasse: leur ligne au catalogue
            //    est la verite, aucun mapping ne doit la couvrir.
            const propre = depuisHistorique(produit, dateIso);
            if (propre != null) return { prix: propre, origine: 'prix propre' };

            // 2. Le MAPPING PRODUITS: le libelle designe une entree du
            //    catalogue, dans une unite qui peut differer de la sienne.
            const a = aliasParNom.get(normaliser(produit));
            if (a) {
                const base = depuisHistorique(a.cible, dateIso);
                if (base != null) {
            ...
            // 3. Regle metier codee en dur, et qui le reste: toute la famille
            //    boeuf/veau partage le cout de la carcasse. C'est une identite
            //    d'espece - « Boeuf en detail » EST du boeuf - la ou le
            //    mapping exprime un choix de decoupe propre a chaque point de
            //    vente. Elle sert de repli quand rien n'est mappe.
