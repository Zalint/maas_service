 ...
 */
/**
 * CLE D'UNE APPROBATION MANUELLE: date + montant.
 *
 * Porter l'approbation sur la seule DATE la ferait survivre a une cloture
 * rectifiee: un depot corrige de 200 000 a 500 000 resterait approuve sans
 * que personne ne l'ait revu. Avec le montant, l'approbation TOMBE quand le
 * chiffre change, et la ligne redevient rouge - ce qui force a la regarder.
 */
function cleApprobation(date, montant) {
    return String(date || '').slice(0, 10) + '|' + round2(montant);
}

function apparierDepots(depots, remboursements, tolerance, approuves) {
    // Les approbations manuelles: l'exploitant a constate que le versement
    // existe, meme si le rapprochement automatique ne l'a pas trouve. Un
    // depot approuve n'est PAS apparie - on ne lui invente pas de
    // remboursement - mais il sort du montant a soustraire.
    const approuvesSet = approuves instanceof Set
        ? approuves
        : new Set(Array.isArray(approuves)
            ? approuves.map((a) => cleApprobation(a.date, a.montant))
            : []);
    ...
    const appariements = (depots || []).map((dep) => {
        ...
        const approuve = !choisi && approuvesSet.has(cleApprobation(date, montant));
        return {
            date: date,
            montant: montant,
            apparie: !!choisi,
            // Approuve a la main: ni retrouve, ni soustrait.
            approuve: approuve,
    ...
    // Ce qui reste VRAIMENT a soustraire: ni retrouve, ni approuve.
    const nonApparies = appariements.filter((a) => !a.apparie && !a.approuve);
    const approuvesLignes = appariements.filter((a) => a.approuve);
    return {
        appariements: appariements,
        nb_apparies: appariements.filter((a) => a.apparie).length,
        nb_approuves: approuvesLignes.length,
        total_approuve: round2(approuvesLignes.reduce((s, a) => s + a.montant, 0)),
...
function construireCashTheorique(a) {
    ...
    // LES LIGNES « AUTRES »: ce que le modele ne sait pas nommer.
    //
    // Montant SIGNE - positif pour une entree, negatif pour une sortie - et
    // un commentaire obligatoire cote saisie. On somme ici, mais on garde le
    // detail: un total de 38 000 sans le detail ne s'explique plus au bout
    // d'un mois.
    const autres = (args.autres || []).map((x) => ({
        id: x.id,
        montant: round2(x.montant),
        commentaire: String(x.commentaire || '')
    }));
    const totalAutres = round2(autres.reduce((s, x) => s + x.montant, 0));
    ...
    if (!rapprochement.appariements.length) {
        ...
        commentaire = 'Les ' + rapprochement.appariements.length + ' dépôts Mata de la période '
            + 'sont couverts : ' + rapprochement.nb_apparies + ' retrouvés dans les '
            + 'remboursements de créance'
            + (rapprochement.nb_approuves
                ? ', ' + rapprochement.nb_approuves + ' approuvés à la main'
                : '')
            + '. Aucun n\'est soustrait une seconde fois.';
    } else {
        ...
        if (rapprochement.nb_approuves) {
            commentaire += ' ' + rapprochement.nb_approuves + ' autre(s) ont été '
                + 'approuvés à la main pour ' + fmtF(rapprochement.total_approuve)
                + ' et ne sont donc pas soustraits.';
        }
    ...
    return {
        lignes: [
            ...
            // Le signe vient du MONTANT, pas d'une regle fixe: la ligne sert
            // aux deux sens, et l'ecran colore d'apres `signe`.
            { cle: 'autres', libelle: 'Autres', montant: Math.abs(totalAutres),
                signe: totalAutres < 0 ? -1 : 1 }
        ],
        autres: autres,
...
module.exports = { apparierDepots, construireCashTheorique, cleApprobation };
