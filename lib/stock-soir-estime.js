...
function estimerStockSoir(args) {
    ...
    const ajouterVente = (produit, quantite) => {
        ...
        // La graphie D'ORIGINE de la cible, gardee pour les messages. La cle
        // normalisee est sans accent et en minuscules: un avertissement qui
        // renvoie l'utilisateur vers la colonne « Mappe vers » doit nommer le
        // produit comme le mapping l'ecrit, pas comme le module l'indexe.
        if (!libelleCible.has(cle)) libelleCible.set(cle, cible);
    ...
    for (const [cle, q] of ventesParProduit) {
        if (q > SEUIL_KG && !lignesConnues.has(cle)) {
            orphelines.push(`${libelleCible.get(cle) || cle} (${arrondi(q)})`);
        }
    }
    ...
    const lignes = lignesAncre.map((l) => {
        ...
        // Calcule UNE fois: la quantite et le detail affiche lisaient le meme
        // mouvement par deux appels distincts, qui ne pouvaient que diverger
        // si l'un des deux changeait.
        const mvtTransferts = mouvementTransferts(
            transfertsParProduit.get(cleP), pool === POOL_DECHET);
        ...
        return {
            ...
            calcul: {
                ...
                transferts: arrondi(mvtTransferts),
