...
async function creerResolveurPrixAchatSimulation({ dateMax, reglages }) {
    ...
    // nom normalise -> POIDS unitaire. Le cout vaut poids x prix carcasse: le
    // Jarret se vend a la PIECE quand la carcasse se paie au KILO, et leur
    // appliquer le meme prix par unite donnait -3 557 F de marge sur un
    // produit vendu 500 F.
    //
    // Une entree en chaine simple vaut poids 1 - le produit se compte alors
    // dans la meme unite que la carcasse.
    const poidsBoeuf = new Map(
        ((reglages && reglages.familleBoeuf) || [])
            .map((e) => (typeof e === 'string' ? { nom: e, poids: 1 } : e))
            .filter((e) => e && e.nom)
            .map((e) => [normaliserNom(e.nom), Number(e.poids) > 0 ? Number(e.poids) : 1])
    );
    ...
    const pourDate = (dateIso) => {
        ...
        const prixAchat = (produit) => {
            ...
            // que rien ne le dise.
            const poids = poidsBoeuf.get(normaliserNom(produit));
            if (poids !== undefined) {
                if (prixBoeuf === null) { noterSansCout(produit, SOURCE_BOEUF); return null; }
                return poids * prixBoeuf;
            }
        ...
        const origine = (produit) => {
            const propre = parseFloat(r.prixAchat(produit));
            if (Number.isFinite(propre) && propre > 0) return 'propre';
            if (poidsBoeuf.has(normaliserNom(produit))) {
