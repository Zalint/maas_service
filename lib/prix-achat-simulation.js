...
async function creerResolveurPrixAchatSimulation({ dateMax }) {
    ...
    const pourDate = (dateIso) => {
         ...
         * « prix propre », « mappé vers Boeuf × 0,5 », ou « sources multiples ». La
        ...
        // cibleDuCout est RELAYEE telle quelle. Sans elle, l'appelant ne
        // sait pas que « Boeuf en gros » et « Boeuf en détail » partagent la
        // meme carcasse, et chacun moyenne son cout sur son propre calendrier
        // de vente - deux couts pour une seule carcasse.
        return {
            prixAchat, origine,
            cibleDuCout: r.cibleDuCout,
            prixAchatDefaut: r.prixAchatDefaut,
            origineBoeuf: r.origineBoeuf
        };
    };
