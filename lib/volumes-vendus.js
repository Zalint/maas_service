...
function agregerVolumes(lignes) {
    ...
    for (const l of (lignes || [])) {
        ...
        totalLignes += 1;
        if (nomBrut === null || nomBrut === undefined || String(nomBrut).trim() === '') continue;
    ...
    const produits = Array.from(parCle.values()).map((a) => ({
        ...
        quantite: arrondi2(a.quantite),
        ca: arrondi2(a.ca),
        nb_lignes: a.nb_lignes,
        // Prix MOYEN constate, pas prix de catalogue: c'est celui-la qui
        // explique le chiffre d'affaires de la periode.
        prix_moyen: a.quantite > 0 ? arrondi2(a.ca / a.quantite) : null
    }));
    ...
    return {
        produits,
        total_ca: arrondi2(totalCa),
        total_quantite: arrondi2(totalQuantite),
