...
function _parNomDepuisCatalogue(catalogue) {
    ...
    for (const p of catalogue || []) {
        ...
        if (!Number.isFinite(prixMaas) || prixMaas <= 0) {
            // Un produit qui a un prix de VENTE mais pas de prix facture: ce
            // cas-la merite d'etre compte, a la difference d'un produit pas
            // tarife du tout (ni l'un ni l'autre), qui n'a rien d'anormal.
            // DEUX causes possibles, et on ne peut pas les distinguer d'ici:
            // le prix d'ACHAT n'a pas ete saisi cote MATA (prixAchat null,
            // donc prixMaas null), ou la version de DATA n'expose pas encore
            // le champ. Le message ne doit donc en affirmer aucune.
...
async function getPrixVenteMaasParNom(dateISO) {
    ...
    const promise = (async () => {
        ...
        try {
            ...
            // Des produits tarifes en vente mais sans prix facture. On ne se
            // replie pas sur `prix` (hors commission), on le DIT: ces lignes
            // restent modifiables a la main, gardent leur commission, et
            // l'ecran doit pouvoir expliquer pourquoi elles ne sont pas
            // verrouillees.
            if (sansPrixMaas > 0) {
                console.warn(`⚠️  DATA prix-vente-maas: ${sansPrixMaas} produit(s) ont un prix de `
                    + "vente mais aucun prix facturé (prix d'achat non saisi côté MATA, ou "
                    + 'version de DATA sans ce champ) — ces lignes gardent leur prix enregistré '
                    + 'et leur commission.');
            }
