 ...
 *   - clé x-api-key = WEB_ORDERS_API_KEY, sinon DATA_API_KEY. Elle DOIT être
 *     égale à EXTERNAL_API_KEY de DATA (celle que valident les routes
 *     /api/external/weborders*, la même que n8n envoie). NB: on n'utilise PAS
 *     EXTERNAL_API_KEY de Maas — c'est une clé propre à Maas, valeur différente.
 *
 * Comportement:
...
function getConfig() {
    ...
    // NB: on N'utilise PAS EXTERNAL_API_KEY de Maas comme fallback: c'est la
    // clé propre de Maas (autres consommateurs), et sa valeur diffère de celle
    // que DATA attend. La clé doit égaler EXTERNAL_API_KEY de DATA (n8n).
    const apiKey = (
...
async function listCommandesWeb(opts) {
    ...
    if (!opts.bypassCache && cached && (now - cached.fetchedAt) < LIST_CACHE_TTL_MS) {
        return cached.data;
    }

    // Lance (ou réutilise) un fetch dédupliqué. Succès -> met à jour le cache.
    // Échec -> conserve le stale (le cache n'est jamais écrasé sur erreur).
    const refresh = () => {
    ...
    // Stale-while-revalidate: cache expiré dispo (et pas bypass) -> renvoyer le
    // stale tout de suite et rafraîchir en fond (un échec garde le stale).
    if (!opts.bypassCache && cached) {
        refresh().catch(() => {});
        return cached.data;
    }

    // Cache miss ou bypassCache -> on attend le fetch.
    return refresh();
}
