...
/** Cumule q et son COUT REEL (pa/div * q, pas une soustraction CA - marge)
 * sous la graphie NORMALISEE de nom, en gardant la premiere graphie vue pour
 * l'affichage. coutLigne est null quand le prix d'achat de cette ligne est
 * inconnu: le produit reste visible (sa quantite compte) mais son cout est
 * marque incomplet plutot que tu par un zero qui se lirait comme "gratuit". */
function accumulerProduit(map, nom, q, coutLigne) {
    const cle = normaliserNom(nom);
    const cur = map.get(cle) || { produit: nom, quantite: 0, cout: 0, coutIncomplet: false };
    cur.quantite += q;
    if (coutLigne === null) cur.coutIncomplet = true;
    else cur.cout += coutLigne;
    map.set(cle, cur);
}
...
// produit precisement a couté cher.
//
...
function detailProduits(parProduit, estBoucherie) {
    return Array.from(parProduit.values())
        .map(({ produit, quantite, cout, coutIncomplet }) => ({
            produit: produit,
...
function agregerCommandes(args) {
    ...
    for (const l of lignes) {
        ...
        if (!Number.isFinite(pa) || pa <= 0) {
            ...
            accumulerProduit(e.parProduit, l.produit, q, null);
        ...
        accumulerProduit(e.parProduit, l.produit, q, coutLigne);
...
function agregerClients(args) {
    ...
    for (const l of lignes) {
        ...
        if (!Number.isFinite(pa) || pa <= 0) {
            ...
            accumulerProduit(e.parProduit, l.produit, q, null);
            if (ec) accumulerProduit(ec.parProduit, l.produit, q, null);
            if (e.sans_cout.indexOf(l.produit) < 0) e.sans_cout.push(l.produit);
            sansCout += ca;
            e.ca_chiffre += ca;
            if (ec) ec.ca_chiffre += ca;
            continue;
        }
        const d = diviseurDe(paragePour, paragePct, l.produit, estBoucherie);
        const coutLigne = (pa / d) * q;
        accumulerProduit(e.parProduit, l.produit, q, coutLigne);
        if (ec) accumulerProduit(ec.parProduit, l.produit, q, coutLigne);
        const pv = q > 0 ? ca / q : nb(l.prix_unit);
