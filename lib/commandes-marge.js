...
/** Cumule q sous la graphie NORMALISEE de nom, en gardant la premiere graphie
 * vue pour l'affichage. */
function accumulerProduit(map, nom, q) {
    const cle = normaliserNom(nom);
    const cur = map.get(cle) || { produit: nom, quantite: 0 };
    cur.quantite += q;
    map.set(cle, cur);
}

// LE DETAIL PAR PRODUIT d'une commande, avec son unite.
//
...
function agregerCommandes(args) {
    ...
    for (const l of lignes) {
        ...
        e.lignes += 1;
        e.quantite += q;
        accumulerProduit(e.parProduit, l.produit, q);
...
function agregerClients(args) {
    ...
    for (const l of lignes) {
        ...
        if (ec) {
            ec.ca += ca;
            accumulerProduit(ec.parProduit, l.produit, q);
        }
