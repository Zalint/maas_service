 ...
 */
/**
 * Le texte stocke -> l'objet exploitable. PUR, donc partage par la lecture
 * simple et par la lecture sous verrou, qui doivent normaliser a l'identique.
 *
 * On RENORMALISE a chaque lecture. Une ligne ecrite a la main en base, ou par
 * une version anterieure, ne doit pas pouvoir faire disparaitre une journee
 * des totaux sous une forme que l'ecriture aurait refusee.
 */
...
async function lireExclusions() {
    ...
    try {
        ...
        return normaliser(row && row.value);
    } catch (e) {
...
async function basculerExclusion(args) {
    ...
    // LIRE-MODIFIER-ECRIRE SOUS VERROU. Tout le mois tient dans une seule
    // ligne de finance_config: sans verrou, deux personnes qui cochent deux
    // journees differentes en meme temps lisent le meme etat de depart, et la
    // seconde ecriture ecrase la premiere. L'exclusion perdue ne dit rien -
    // la case se decoche au prochain chargement, sans explication.
    //
    // La ligne est CREEE avant d'etre verrouillee: SELECT ... FOR UPDATE ne
    // verrouille que des lignes existantes, et sur une table vide les deux
    // transactions passeraient sans se voir. findOrCreate porte cette
    // creation, et sa contrainte de cle primaire arbitre si les deux la
    // tentent ensemble.
    const { FinanceConfig, sequelize } = require('../db/models');
    return sequelize.transaction(async (t) => {
        await FinanceConfig.findOrCreate({
            where: { key: CLE },
            defaults: { value: '{}', updated_at: new Date() },
            transaction: t
        });
        const row = await FinanceConfig.findOne({
            where: { key: CLE }, transaction: t, lock: t.LOCK.UPDATE
        });
        const toutes = normaliser(row && row.value);
        ...
        // cette purge, la ligne grossit indefiniment: personne ne va reintegrer
        // a la main une journee de 2024 pour faire de la place.
        ...
        await FinanceConfig.update(
            { value: JSON.stringify(toutes), updated_at: new Date() },
            { where: { key: CLE }, transaction: t }
        );
...
module.exports = {
    lireExclusions, basculerExclusion, normaliser,
    CLE, FORME_MOIS, MAX_PAR_MOIS, MAX_MOIS
};
