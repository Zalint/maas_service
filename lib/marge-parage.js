...
// SEUIL_KG vient de ./parage, jamais recopie: sous ce seuil, aucun prix moyen
// n'a de sens - diviser un montant par une poussiere de kilos produirait un
// prix au kilo astronomique.
const { quantiteEnKg, compositionDuPack, normaliserNom, CATEGORIES, SEUIL_KG } = require('./parage');
...
function calculerMarge(args) {
    ...
    for (const cat of CATEGORIES) {
        ...
        for (const [produit, l] of Object.entries(parProduit)) {
            ...
            // Un theorique NEGATIF veut dire que le stock du soir depasse
            // matin + transferts: le produit est ENTRE en stock, il n'en est
            // pas sorti. C'est le cas du dechet, cree par la decoupe. Le
            // couter reviendrait a soustraire du cout et a fausser
            // prix_achat_moyen, dont le denominateur diminuerait lui aussi.
            // On l'ecarte et on le NOMME: c'est une anomalie de saisie ou un
            // sous-produit mal categorise, pas un cout negatif.
            if (theo < 0) {
                kgNegatifs.push({ produit, kg: theo });
                continue;
            }
        ...
        resultat[cat] = {
            ...
            // Les kilos reellement valorises. Publie tel quel plutot que
            // reconstruit ailleurs par difference: la soustraction
            // `theorique - kg_sans_prix_achat` restait juste tant que ces deux
            // seuls postes existaient, et a cesse de l'etre des qu'un troisieme
            // - le theorique negatif - est venu s'y glisser.
            kg_coutes: kgCoutes,
            // UNIQUEMENT les kilos dont le prix d'achat est inconnu. Ni les
            // kilos negatifs, ni un reste calcule: ce nombre porte une seule
            // cause, et reste donc lisible tel qu'il s'appelle.
            kg_sans_prix_achat: kgSansPrix,
            produits_sans_prix_achat: sansPrix,
            // Produits dont le stock a AUGMENTE sur la periode: ecartes du cout.
            kgNegatifs,
