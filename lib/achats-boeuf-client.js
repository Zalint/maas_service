 ...
 * Chaque date = coût total du lot / poids total (moyenne pondérée par les kg).
...
async function _loadRows(bypassCache) {
    ...
    _inflight = (async () => {
        ...
        try {
            ...
            // Agréger par date: bœuf uniquement, coût total / poids total.
            // Le coût est recalculé depuis prix/abats/frais_abattage (definition
            // du prix de revient). Equivalent a Σ(prix_achat_kg × nbr_kg), les
            // deux coincident au centime pres sur les donnees reelles.
            // Une bete sans poids ou sans cout exploitable est ignoree: l'API
            // renvoie `parseFloat(x) || 0`, donc une valeur manquante arrive en
            // 0 et fausserait le prix de revient du lot.
            ...
            for (const a of achats) {
                ...
                const cost = (parseFloat(a.prix) || 0)
